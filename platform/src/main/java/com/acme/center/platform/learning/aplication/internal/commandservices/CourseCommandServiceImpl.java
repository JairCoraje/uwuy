    ...
    public void handle(AddTutorialToCourseLearningPathCommand command){
        ...
        try{
            courseRepository.findById(command.courseId()).map(course -> {
                ...
                courseRepository.save(course);
                System.out.println("Tutorial added to course learning path");
                return course;
            });
        }catch (Exception e){
            throw new IllegalArgumentException("Error while adding tutorial to course learning path: " + e.getMessage());
        }
    }
}
